...
description: 'Run a design-challenging Claude Code review of local git changes in this repository. Args: --wait, --background, --base ref, --scope auto|working-tree|branch, --model model, --effort low|medium|high|xhigh|max, [focus text]. Defaults to Claude Fable 5.1 (claude-fable-5-1) with no forced effort. Use only when the user wants stronger scrutiny than a normal review, such as explicit tradeoff challenge, risky-change review, or custom focus text.'
---
...
Supported arguments: `--wait`, `--background`, `--base <ref>`, `--scope auto|working-tree|branch`, `--model <model>`, `--effort <low|medium|high|xhigh|max>`, plus optional focus text after the flags (defaults: model=claude-fable-5-1 and no effort; `fable`, `opus`, `sonnet`, and `haiku` each keep Claude Code's own effort default, and Claude Code owns which effort levels each model supports)
